...
void Mismatch_Scan_With_ScoreX(char* Ref,char* Read,int Read_Len,int Max_Mis,int Current_Mis,Alignment & A)
{
	int Mis=0;
	int SW_Score=0;
...
Alignment Fast_SW(char *Ref,char *Read,char *Quality,int OFF,char Sign)
{
	...
	if(Gap_Alignment.Score>0 && Gap_Alignment.Score!=INT_MAX)
	{
		...
		if(Gap_Alignment.B)//Indel present
		{
			...
			{
				Mismatch_Scan_With_Score(Ref+Skip,Read+Skip+Gap_Alignment.B,Quality+Skip+Gap_Alignment.B,Tot_Read_Len-Skip-Gap_Alignment.B,100,0,A);
				A.Loc+=Gap_Alignment.B;
			}
			else
			{
				assert(Gap_Alignment.Indel_Flag=='D');
				Mismatch_Scan_With_Score(Ref+Skip+Gap_Alignment.B,Read+Skip,Quality+Skip,Tot_Read_Len-Skip,100,0,A);
				A.Loc-=Gap_Alignment.B;
			}
...
Alignment Fast_SWX(char *Ref,char *Read,int OFF)
{
	...
	if(Gap_Alignment.Score>0 && Gap_Alignment.Score!=INT_MAX)
	{
		...
		{
			int Skip=Tot_Read_Len-Half_Length+Gap_Alignment.A;//skip from begin..
			Mismatch_Scan_With_ScoreX(Ref,Read,Skip,100,0,A);//left half..
			if(Gap_Alignment.Indel_Flag=='I')
			{
				Mismatch_Scan_With_ScoreX(Ref+Skip,Read+Skip+Gap_Alignment.B,Tot_Read_Len-Skip-Gap_Alignment.B,100,0,A);
			}
			else
			{
				assert(Gap_Alignment.Indel_Flag=='D');
				//Mismatch_Scan_With_Score(Ref+Skip+Gap_Alignment.B,Read+Skip,Quality,Tot_Read_Len-Skip+Gap_Alignment.B,100,0,A);
				Mismatch_Scan_With_ScoreX(Ref+Skip+Gap_Alignment.B,Read+Skip,Tot_Read_Len-Skip,100,0,A);
			}
			A.BQScore-=(BOPEN+BEXT*(Gap_Alignment.B));
			A.SW_Score-=(gap_open+Gap_Alignment.B*gap_extension);
			A.Score+=(gap_openP+Gap_Alignment.B*gap_extensionP);
		...
		{
			Mismatch_Scan_With_ScoreX(Ref,Read,Tot_Read_Len,100,0,A);
			sprintf(A.Cigar,"%dM",Gap_Alignment.A);
		}
		A.Score= -A.Score;
	}
